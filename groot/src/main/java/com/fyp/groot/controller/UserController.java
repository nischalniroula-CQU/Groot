...
public class UserController {
    ...
    public ResponseEntity<UserLoginResponse> login(@RequestBody UserLoginRequest loginRequest) throws Exception {
        try {
            // Attempt to retrieve the user by email from Firebase
            ...
            User localUser = userService.findByFirebaseId(userRecord.getUid())
                    .orElseGet(() -> {
                        ...
                        newUser.setFirebaseId(userRecord.getUid());
            ...
            if (localUser.getIsActive() && (localUser.getRole() == Role.owner() || userRecord.isEmailVerified())) {
                // Generate custom token for user
                String customToken = FirebaseAuth.getInstance().createCustomToken(userRecord.getUid());

                // Prepare login response
                UserLoginResponse response = new UserLoginResponse(null, null, customToken, userRecord.getEmail(), userRecord.getUid());
                response.setMessage(response.getMessage());
                response.setStatus(response.getStatus());
                response.setIdToken(customToken);
                ...
                throw new Exception("User is not authorized to log in.");
            }
